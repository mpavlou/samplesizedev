...
Hence, in our new work we develop sample size calculations where we aim to control the probability of acceptable performance, rather than performance on average. In a simulation-based framework this appraoch can be implemented for any performance metric for a suitably defined range of acceptable performance. In the paper above and in this package we focused on the calibration slope. In addition to the simualation-based approach we also derived an approximate analytical calculation that is unbiased in most cases and very quick. 
