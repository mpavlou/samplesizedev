...
In the paper above we evaluated existing sample size formulae (Riley et al. (2019)) for the development of risk models. The formulae concern 3 criteria: 
- C1 control overfitting (target: calibration slope=0.9),
- C2 control optimism in R2 Nageleherke (target: opt=0.05)
- C3 precision in the mean predicted risk (target: precision = 0.05).

The formula which aims to control model overfitting (calibration formula - C1) most often gives that highest sample size and in our article we focused our detailed evaluations around this formulae. While the calibration formula performed relatively well for models with C-statistic/C-index<0.8, it substantially ***underestimated*** the sample size when the predictive strength of the model was higher. The sample sizes often needed to be increased by 50% or even doubled to meet the calibration targets.
