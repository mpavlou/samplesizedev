expected_s_n_binary_quick <- function(n, S, mean_eta, variance_eta,  p, c, n.predictors, beta, nsim = 1000, nval = 25000, parallel = TRUE){
  ...
  a <- foreach::foreach(i=1:nsim,  .packages=c('mvtnorm','RcppNumerical', 'ggplot2','stats')) %dopar% {
  ...
  cs_plot   <- ggplot2:: ggplot(df,  ggplot2::aes(x = cs), size=12) +
    ggplot2::geom_density() +  ggplot2::ggtitle(paste("N = ", round(n), ", Expected CS = ", round(mean(cs,na.rm=TRUE),3), ", SD(CS) = ", round(sqrt(stats::var(cs,na.rm=TRUE)),3))) +
