...
#' @param x (logical) Matrix of covariate values
#' @param y (logical) Matrix of outcome values
...
expected_cs_mape_binary_xy <- function(n, p, c, n.predictors, beta, nsim = 1000, nval = 25000, method ="MLE",
  ...
  idx <- sample( N, nval, replace = FALSE)
  xval <- x_large[idx, , drop = FALSE]
  ...
  ncalc       <- 500000
  x           <- x_large[sample( N, ncalc, replace = FALSE), , drop = FALSE]
  ...
  if (approx==TRUE) {
    ...
    ncalc      <- 100000
    idx        <- sample( N, ncalc, replace = FALSE)
    x          <- x_large[idx, , drop = FALSE]
    eta        <- mean+x%*% beta
    y          <- stats::rbinom(ncalc,  1, invlogit(eta))
    fit        <- stats::glm(y~x, family=binomial())
    varbeta    <- stats::vcov(fit)
    sampbeta   <- mvtnorm::rmvnorm(nsim, c(mean, beta), sigma = varbeta*ncalc/n)
  }
  ...
  mape_plot <- ggplot2::ggplot(df,  ggplot2::aes(x = mape), size=12) +
    ...
    ggplot2::theme(text =  ggplot2::element_text(size = 10)) +ggplot2::xlab("MAPE") + ggplot2::theme_bw()+
    ggplot2::theme(axis.text=ggplot2::element_text(size=10),
                   axis.title=ggplot2::element_text(size=10))+
    ggplot2::theme(plot.title =  ggplot2::element_text(size = 10))
  ...
  idval      <- sample( N, round(500000), replace = FALSE)
  xval       <- x_large[idval, , drop = FALSE]
  yval       <- y_large[idval]
  prev      <- mean(yval)
