...
expected_cs_mape_binary_xy <- function(n, p, c, n.predictors, beta, nsim = 1000, nval = 25000, method ="MLE", parallel = TRUE, long = FALSE, approx=FALSE, x=x, y=y){
  ...
  data_bs  <- data
  set.seed(2022)
  for (i in 1: max) data_bs <-rbind(data_bs,data[sample(nrow(x), nrow(x), replace = TRUE),])

  xval <- as.matrix(data_bs[,-1])

  # True R2
  MaxR2      <- 1-(((p^(p))*((1-p)^(1-p)))^2)
  ncalc      <- nval
  x          <- xval
  eta        <- cbind(1,x)%*% as.matrix(beta)
  y          <- stats::rbinom(length(eta),  1, invlogit(eta))
  ...
  if (method== "MLE") {
    ...
    a<- foreach::foreach(i = 1:nsim, .packages=c('mvtnorm','RcppNumerical', 'ggplot2' )) %dopar% {
      ...
      x          <- xval[sample(n, n, replace = TRUE),]
      eta        <- cbind(1,x)%*% beta
      y          <- stats::rbinom(n,  1, invlogit(eta))
      eta_val    <- cbind(1,xval)%*% beta
      yval       <- stats::rbinom(length(eta_val), 1, invlogit(eta_val))
