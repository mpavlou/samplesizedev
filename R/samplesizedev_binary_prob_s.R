...
samplesizedev_binary_prob_s <- function(l_s, u_s, PAP_s, p, c,   n.predictors, beta = rep(1/n.predictors, n.predictors), nval = 25000, nsim = 1000, parallel = TRUE, plot = TRUE, quick){
  ...
  if (c>0.72  & c<=0.73 )       {inflation_f   <- 0.95  ; nc <-   n_analytical*inflation_f }
  if (c>0.73  & c<=0.74 )       {inflation_f   <- 0.95  ; nc <-   n_analytical*inflation_f }
  if (c>0.74  & c<=0.75 )       {inflation_f   <- 0.95  ; nc <-   n_analytical*inflation_f }
