...
#' @param individual_quantile (numeric) A quantile of the probability distribution (default 0.5) to obtain the uncertainty in the corresponding individual predicted probability due to finite sample size


#' @return   Data frame df with the following elements:
#' @return   Input sample size (n)
#' @return   Specified outcome prevalence
#' @return   Specified AUC
...
#' @return   Standard deviation of the Individual predicted risk
...
#'
#'
expected_performance <- function(outcome="Binary", n, phi, c,  p,  gamma = rep(1/p, p), nval = 25000, nsim = 1000, parallel = TRUE, method = "MLE", long=FALSE, approx = FALSE, x=NULL, y=NULL, threshold=phi, individual_quantile=0.5){

  beta          <- gamma
  n.predictors  <- p
  p             <- phi

  if (outcome=="Binary" & length(x)==0)   performance <- expected_cs_mape_binary (n=n,  p=p, c=c,  n.predictors = n.predictors, beta=beta, nval = nval, nsim = nsim, parallel = parallel, method = method, long = long, approx = approx, threshold = threshold, individual_quantile = individual_quantile)
  if (outcome=="Binary" & length(x)!=0)   performance <- expected_cs_mape_binary_xy (n=n,  p=p, c=c,  n.predictors = n.predictors, beta=beta, nval = nval, nsim = nsim, parallel = parallel, method = method, long = long, approx = approx, x=x, y=y, threshold = threshold, individual_quantile = individual_quantile)
